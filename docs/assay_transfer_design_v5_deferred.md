# Assay Transfer Version 5: Deferred Extensions to Soft-Evidence Supervision
...
Version 4 freezes the minimal LM-only soft-evidence design: empirical A/B/C vote
fractions, soft-target cross-entropy, no-majority training rows, and the unchanged hard
A/B benchmark.

This document preserves ideas deliberately excluded from V4. None is an implementation
requirement or a permitted silent change to the V4 target.

## 2. Final-decision probability targets

V4 predicts the empirical distribution of record-level evidence. It does not claim that
`q_C` is the probability of an overall no-majority conclusion.
...
These methods are deferred because V4 first needs to establish whether empirical soft
supervision improves raw LM logits.
...
be compared with the V4 equal-record reference.

## 5. Scalar projections

One-number projections may be useful for ablations but are not canonical in V4.
...
## 8. Sampling, weighting, and curriculum
...
An extension may leave V5 deferral only when:
...
- V3 binary and V4 empirical-distribution artifacts remain usable unchanged; and
