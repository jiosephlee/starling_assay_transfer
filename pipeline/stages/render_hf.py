...
from pipeline.variance_soft import (
    ...
    completion as variance_completion,
    target_distribution as variance_target_distribution,
    target_temperature,
)
...
def _target_type(binary: bool = False) -> pa.StructType:
    names = ("transfer", "nontransfer") if binary else (
        "transfer", "nontransfer", "ambiguous")
    return pa.struct([pa.field(name, pa.float32()) for name in names])


def _metadata_type(soft_evidence: bool = False,
                   variance_soft_binary: bool = False) -> pa.StructType:
    ...
    target_enabled = soft_evidence or variance_soft_binary
    if target_enabled:
    ...
    if target_enabled:
        nested.append(pa.field("target_distribution", _target_type(variance_soft_binary)))
    if variance_soft_binary:
        nested.extend([pa.field("target_temperature", pa.float32()),
                       pa.field("completion_is_tie_anchor", pa.bool_())])
...
def _hf_schema(soft_evidence: bool, variance_soft_binary: bool = False) -> pa.Schema:
    fields = [pa.field("prompt", pa.large_string()), pa.field("completion", pa.string())]
    fields.append(pa.field("metadata", _metadata_type(soft_evidence, variance_soft_binary)))
    return pa.schema(fields)


def _load_templates(template_dir: Path, binary_choices: bool = False) -> dict[str, Template]:
    def source(concept: str) -> str:
        text = (template_dir / f"{concept}.jinja").read_text()
        return text.replace("\n(C) ambiguous", "") if binary_choices else text

    return {concept: Template(source(concept),
...
def _metadata(row: dict[str, Any], template_id: str, schema_version: str,
              soft_evidence: bool = False, target_policy_version: str | None = None,
              variance_soft_binary: bool = False) -> dict[str, Any]:
    ...
    meta.update({"schema_version": schema_version, "template_id": template_id,
                 "evidence": _evidence(row, target_enabled), "retrieval_context": _context(row),
                 "provenance": _provenance(row)})
    if target_enabled:
        meta["target_policy_version"] = target_policy_version
        meta["target_distribution"] = (variance_target_distribution(row)
                                               if variance_soft_binary else target_distribution(row))
    if variance_soft_binary:
        meta["target_temperature"] = target_temperature(row)
        meta["completion_is_tie_anchor"] = completion_is_tie_anchor(row)
    return meta
...
def _render(row: dict[str, Any], templates: dict[str, Template], variant: str = "standard",
    ...
    if not soft_evidence and not variance_soft_binary and label not in (0, 1):
        raise ValueError(f"invalid binary label for {row.get('candidate_id')}: {label}")
    concept = row["assay_concept"]
    version = "v5" if variance_soft_binary else ("v4" if soft_evidence else "v3")
    ...
    if variance_soft_binary:
        completion = variance_completion(row)
    else:
    ...
    record = {"prompt": templates[concept].render(row=row), "completion": labels[completion],
              "metadata": _metadata(row, template_id, schema_version, soft_evidence,
                                    target_policy_version, variance_soft_binary)}
    return record
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    variance_soft_binary = bool(getattr(args, "variance_soft_binary", False))
    templates = _load_templates(args.template_dir, variance_soft_binary)
    ...
    if soft_evidence and variance_soft_binary:
        raise ValueError("V4 and V5 target modes are mutually exclusive")
    if (soft_evidence or variance_soft_binary) and not target_version:
    ...
    for split, records in by_split.items():
        ...
        pq.write_table(pa.Table.from_pylist(
            records, schema=_hf_schema(soft_evidence, variance_soft_binary)),
    ...
    completion_map = ({"rule": "count_argmax_a_on_exact_tie"} if variance_soft_binary else
                      ({"rule": "unique_argmax_c_on_tie"} if soft_evidence else
                      ({"1": "(A)", "0": "(B)"} if variant == "intern" else
                       {"1": "A", "0": "B"})))
    ...
    if soft_evidence or variance_soft_binary:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--variance-soft-binary", action="store_true")
