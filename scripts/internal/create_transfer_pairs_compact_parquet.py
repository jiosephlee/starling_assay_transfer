...
def load_records(args: argparse.Namespace) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    requested.update(args.same_metadata_columns)
        ...
        records.append(
            {
                ...
                "same_metadata": {column: row.get(column) for column in args.same_metadata_columns},
                "_fp": fp,
            }
        )
...
def all_group_pair_indices(groups: list[dict[str, Any]]) -> Iterator[tuple[int, int]]:
    for group in groups:
...
def same_metadata_key(record: dict[str, Any], columns: list[str]) -> tuple[Any, ...] | None:
    if not columns:
        return ()
    values: list[Any] = []
    for column in columns:
        value = record["same_metadata"].get(column)
...
def group_records_by_same_metadata(
    records: list[dict[str, Any]],
    columns: list[str],
        ...
        return records, [
            {
                ...
                "end": len(records),
                "records": len(records),
                "candidate_pairs": len(records) * (len(records) - 1) // 2,
            }
        ]
    grouped: dict[tuple[Any, ...], list[dict[str, Any]]] = {}
    for record in records:
        key = same_metadata_key(record, columns)
        if key is None:
            continue
    ...
    start = 0
    for key, group in sorted(grouped.items(), key=lambda item: (-len(item[1]), repr(item[0]))):
        if len(group) < 2:
            continue
        group = sorted(group, key=lambda record: int(record["row_index"]))
        end = start + len(group)
        ...
        start = end
    return sorted_records, group_metadata


def left_ranges_for_group(
    *,
    start: int,
    end: int,
    n_ranges: int,
    ...
    if n < 2:
        return []
    local_ranges = left_ranges_by_pair_count(n, n_ranges)
    return [(start + local_start, start + local_end, end) for local_start, local_end in local_ranges]


def grouped_left_ranges_by_pair_count(groups: list[dict[str, Any]], n_ranges: int) -> list[tuple[int, int, int]]:
    total_pairs = sum(int(group["candidate_pairs"]) for group in groups)
    if total_pairs <= 0:
        return []
    target = max(1, math.ceil(total_pairs / max(1, n_ranges)))
    ranges: list[tuple[int, int, int]] = []
    for group in groups:
        group_pairs = int(group["candidate_pairs"])
        group_ranges = max(1, math.ceil(group_pairs / target))
        ranges.extend(
            left_ranges_for_group(
                start=int(group["start"]),
                end=int(group["end"]),
                n_ranges=group_ranges,
            )
        )
    return ranges
...
def process_left_range(
    ...
    with pq.ParquetWriter(
        ...
        for left_idx in range(start, end):
            ...
            for right_idx in range(left_idx + 1, right_end):
                candidates_seen += 1
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.same_metadata_columns and not args.enumerate_all:
        raise ValueError("--same-metadata-columns is currently supported only with --enumerate-all")
    records, same_metadata_groups = group_records_by_same_metadata(records, args.same_metadata_columns)
    n = len(records)
    if args.same_metadata_columns:
        load_stats = {
            **load_stats,
            "records_excluded_by_same_metadata": loaded_records - n,
            "same_metadata_groups": len(same_metadata_groups),
        }
    if n < 2:
        raise RuntimeError("need at least two valid records after same-metadata filtering")
    total_possible_pairs = sum(int(group["candidate_pairs"]) for group in same_metadata_groups)
    if args.enumerate_all:
        target_candidates = total_possible_pairs
        pair_iter = (
            all_group_pair_indices(same_metadata_groups)
            if args.same_metadata_columns
            else all_pair_indices(n)
        )
        sampling_mode = (
            "deterministic_exhaustive_same_metadata_group_order"
            if args.same_metadata_columns
            else "deterministic_exhaustive_row_index_order"
        )
    else:
    ...
    if args.enumerate_all and args.workers > 1:
        ...
        if args.same_metadata_columns:
            ranges = grouped_left_ranges_by_pair_count(
                same_metadata_groups,
                max(args.workers * args.tasks_per_worker, 1),
            )
        else:
            ranges = [
                (start, end, n)
                for start, end in left_ranges_by_pair_count(
                    n,
                    max(args.workers * args.tasks_per_worker, 1),
                )
            ]
        with ProcessPoolExecutor(
            ...
            futures = []
            for shard_id, (start, end, right_end) in enumerate(ranges):
                futures.append(
                    executor.submit(
                        process_left_range,
                        shard_id,
    ...
    metadata = {
        ...
        "same_metadata_columns": args.same_metadata_columns,
        "same_metadata_policy": (
            "only_non_null_exact_matches_within_same_metadata_key"
            if args.same_metadata_columns
            else None
        ),
        "same_metadata_groups": same_metadata_groups if args.same_metadata_columns else [],
        "valid_records": n,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Restrict exhaustive pair generation to records with identical non-null values "
            "for these columns."
        ),
    )
    ...
    if args.same_metadata_columns and not args.enumerate_all:
        parser.error("--same-metadata-columns currently requires --enumerate-all")
    if any(not column.strip() for column in args.same_metadata_columns):
        parser.error("--same-metadata-columns cannot contain blank column names")
